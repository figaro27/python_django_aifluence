...
Message.prototype.renderMessage = function (message, setAsFirst) {
    // check if this message status is sent or received.
    var isSent;
    var selfId = document.querySelector('#chat_id').value;
    selfId = parseInt(selfId.replace(/,/g, ''));
    ...
    if (isSent) {
        elem.classList.add('message__sent');
    }
    else {
        elem.classList.add('message__received');
    }
    ...
    // create div element which includes elm
    var row = document.createElement('div');
    if (selfId == message.sender_id) {
        row.classList.add('row_sent');
    }
    ...
    if (setAsFirst) {
        ...
        if (typingElem) {
            self.container.insertBefore(row, typingElem);
        } else {
        ...
        self.container.insertBefore(row, self.container.firstElementChild);
    ...
    if(!currentDialog.tplDateMessage[month+'/'+ date.getDate()] ||
        ...
        self.container.insertBefore(elemDate,row);
    }
...
Message.prototype.setMessageStatus = function(data) {
    ...
    if(dialogId === dialogModule.dialogId){
        ...
        if(messageElem !== undefined && status == 'read'){
            // hide eye icon of last read message
            var checkIcons = document.querySelectorAll('.check');
            for (var i = checkIcons.length - 1 ; i >= 0 ; i--) {
                var checkIcon = checkIcons[i];
                if (checkIcon.getAttribute('style') == 'display : block') {
                    checkIcon.removeAttribute('style');
                    break;
                }
            }
