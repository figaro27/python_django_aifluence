...
Dialog.prototype.init = function () {
    ...
    self.dialogType = document.querySelector('#type').value;
    self.username = document.querySelector('#username_').value;
...
Dialog.prototype.loadDialogs = function (type) {
    ...
    // QB custom object class filter
    if (self.dialogType) {
        filter['data[class_name]'] = QB_CUSTOM_OBJECT_CLASS['name'];
        filter['data[type]'] = self.dialogType;
    }
...
Dialog.prototype.renderDialog = function (dialog, setAsFirst) {
    ...
    for (var i = 0 ; i < dialogNameArray.length ; i++) {
        if (self.username != dialogNameArray[i]) {
            dialog.name = dialogNameArray[i];
            break;
        }
    }
    // dialog.campaign_detail = 'campaign detail';
