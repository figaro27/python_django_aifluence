...
router.on({
    ...
    '/login': function(){
        if(!loginModule.isLogin) {
            loginModule.renderLoadingPage().then(function(){
                router.navigate('/dashboard');
    ...
    '/dashboard': function(){
        if(!loginModule.isLogin) {
            console.log('router."/dashboard"+++++++++++++++++loginModule notLogin');
            ...
            console.log('router/dashboard++++++++++++++++++++++++++++++++++if (app.isDashboardLoaded)')
            app.loadWelcomeTpl();
            app.sidebar.classList.add('active');
        } else {
            console.log('router/dashboard++++++++++++++++++++++++++++++++++if (!app.isDashboardLoaded)')
            app.renderDashboard('chat');
            dialogModule.loadDialogs('chat').then(function(){

            }).catch(function(err){
                console.error(err);
                // app.isDashboardLoaded = false;
                loginModule.login().then(function(){
                    dialogModule.loadDialogs('chat');
                    // router.navigate('/dashboard');
                }).catch();
            ...
            dialogModule.getDialogById(dialogId).then(function(dialog){
                ...
                if(!currentDialog) {
                    ...
                    userModule.getUsersByIds(currentDialog.users).then(function () {
                        // edit dialog name to show opponent's name. Ex : assume username = 'client1'.    'client1__agent1__10' -> 'agent1'
                        var username = document.querySelector('#username_').value;
                        var dialogNameArray = dialog.name.split('__');
                        for (var i = 0 ; i < dialogNameArray.length ; i++) {
                            if (username != dialogNameArray[i]) {
                                dialog.name = dialogNameArray[i];
                                break;
                            }
                        }
